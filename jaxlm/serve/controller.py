...
# Original source: https://github.com/lm-sys/FastChat/blob/main/fastchat/serve/controller.py
"""
    ...
    def get_worker_address(self, model_name: str):
        if self.dispatch_method == DispatchMethod.LOTTERY:
            ...
            if True:  # Directly return address
                pt = np.random.choice(
                    np.arange(len(worker_names)), p=worker_speeds)
...
async def register_worker(request: Request):
    ...
    controller.register_worker(
        data["worker_name"], data["check_heart_beat"], data.get(
            "worker_status", None)
    )
